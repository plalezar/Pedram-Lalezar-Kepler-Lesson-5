...
// PUT YOUR CODE HERE
const names = ["John Smith", "Daniella Abramowitch", "Victoria Hargowitch"];
...
let trees = [
  { type: "Oak", height: "20M" },
  { type: "Coconut", height: "25M" },
  { type: "Olive", height: "30M" },
];
...
const decimals = [0.45, 0.68, 0.35, 0.74];
...
const startingNums = [2, 4, 5, 8];
function addThreeToAll(numbers) {
  return numbers.map((num) => num + 3);
}
console.log(
  `Q6 addThreeToAll [${startingNums}]: `,
  addThreeToAll(startingNums)
);
...
const baseNums = [2, 3, 5, 6];
function squareAll(nums) {
  return nums.map((num) => num * num);
}
console.log(`Q7 squareAll [${baseNums}]:`, JSON.stringify(squareAll(baseNums)));
...
// PUT YOUR CODE HERE
let users = [
  { username: "John Smith", isAdmin: true },
  { username: "Daniella Abramowitch", isAdmin: false },
  { username: "Victoria Hargowitch", isAdmin: true },
];
...
function pluck(arr, key) {
  return arr.map((users) => users[key]);
}
console.log(pluck(users, "name"));
console.log(pluck(users, "isAdmin"));
...
// PUT YOUR CODE HERE
let numberArray = [, 1, 31, 54, 77, 253, 364, 475, 92];
...
// PUT YOUR CODE HERE
let numArray = [4, 8, 103, 54, 235, 546, 9, 3];
function greaterThan100(numberArray) {
  return numberArray.filter((number) => number > 100);
}
console.log(`Q12 greaterThan100 [${numArray}]:`, greaterThan100(numArray));
...
console.log(nonAdminUsers(users));
...
console.log(countAdminUsers(users));
...
function shorterThanX(arr, maxLength) {
  return arr.filter((str) => str.length < maxLength);
}
console.log(shorterThanX(strings, 20));
console.log(shorterThanX(strings, 30));
...
let masterTypes = [5, "7", "five books", 95, { height: 50 }, ["city"]];
function onlyStrings(arr) {
  return arr.filter((member) => typeof member === "string");
}
console.log(`Q16 onlyStrings [${masterTypes}]:`, onlyStrings(masterTypes));
...
// PUT YOUR CODE HERE
function divisibleByX(arr, x) {
  return arr.find((num) => num % x === 0);
}
...
function startsWithLetter(arr, letter) {
  if (letter.length != 1) return "Error: Letter must be a string of length 1";
  return arr.find((str) => str[0] === letter);
}
